...
11. Program Code: Examples are PF, PT, et3. You can use unit program codes, similar to ARMS program codes, to filter unit tasks. You can assign multiple program codes to the same task. If the task is to be mapped to an ADLS task, it must have a program code of ADLS.
...
4. Add Blocks of Instruction \(BOI\) to the correct training day by clicking the BOI icons and moving them to the appropriate spot on the course build canvas. Every distinct lesson is a BOI.
    ...
    3. All others with a pre-req in or out

    4. All stand-alone BOIs \(no pre-req in or out\)
...
7. Tasks are sub-components of the BOIs. While stil in the BOI Details, add tasks by selecting the Tasks ta2. Search from your master task file using either the Task Code or the   Task Name and select GO. Select the tasks you would like to use by moving them into the Tasks Canvas. If you do not have tasks in your master list, you can also add tasks by selecting Create New Task 
    ...
    !!! NOTE
        Information on building a mater task list is provided in the Help Topic, Mission Training Setup. If the BOI is only one thing, e.g., “BEEG 02 – Egress Lab”, it has no Task. Only the BOI is graded overall. If there is only a single graded item in the BOI, the BOI should not have any tasks at all. If a BOI has multiple things that need to be graded, then add a course task for each graded item.
...
14. Review your course for any errors you may have overlooked during the build. An easy way to review your course material in an accessible format is by generating a Course Details Report. See Help Topic, Course Details.

15. Activate your course after you make any necessary revisions. Training > Mission > Overview > Courses. Select Activate.

    !!! NOTE
        Once you activate a course, you cannot make revisions to that course. To revise, you would need to create a new version of the course and edit the new version. Therefore, you should always review and validate the course before activation.

    The next step

    Once you have activated your course, you may begin enrolling students. See Help Topic,  Managing Courses.

    !!! NOTE
        Testing your course may also help you check the functionality of your course build. Try enrolling a “fake” student to the activated course and follow the normal procedures for grading material and completing the course. Evaluate whether or not your build yields the results you would like to see.
...
3. You can enroll students only in Active courses.
...
9. To manage templates \(add, edit, delete\), click on the Manage Template button at the bottom of the Course Assignment page to open the Manage Training Dates Templates dialog.
    ...
    1. A template must have a name, but the description is optional.

    2. You can change the Start Date and End Dates to adjust the number of months displayed.

    3. If more than 12 months are selected, you can use the navigation arrows \(Prev Year, Prev Month, Next Month, and Next Year\) to view additional months.

    4. Ctrl Click on the individual dates you want to include as training dates for this template.

    5. You can use the day controls \(Sunday through Saturday\) at the top of the dialog to select particular days throughout the time span selected, except for Federal holidays – indicated in yellow \(e.g. selecting Monday will select all Mondays in the time span with the exception of any that are Federal holidays\).

    6. You can manually select Federal holidays by Ctrl Clicking on them.

    7. You can Save or Save As New as necessary.

    8. Clicking the Clear Selections button will clear all selections and reset the Start and End Dates as if you opened the dialog with no template selected.

    9. You can delete templates as necessary by clicking the red X to the right of the name of the template.

    10. Templates can be deleted at any time, even after they have been used to enrol a student in a course.

    11. Once you have selected \(or created\) the appropriate template, click the Close button to close the dialog and return to the Course Assignment page with that template selected in the Template dropdown menu.
...
!!! NOTE
    You cannot move or change a BOI that has been graded.
...
5. If the scheduling requirements change, right click on the BOI and select Edit BOI Dates. From the Edit BOI Dates you can choose to:
    ...
    1. Change the date of the selected BOI and all other BOIs following this BOI will also shift according to the selected date.

    2. Change the date of the selected BOI and will affect no other BOIs.

    3. Move all BOIs from this point onward by the number of training days you specify \(negative training days will move the BOIs backwards\) assuming there are no completed BOIs in the way.

    4. Apply a new template you select to ALL BOIs from the selected BOIs forward providing there are no completed BOIs that wil be affected by the change.

!!! NOTE
    Moving a BOI may break prerequisite links – PEX will warn you before it does this.
...
2. Click on the Trainee’s name you would like to review.
...
6. The Reviewer can be entered by selecting the Squadron and shortname of the reviewer. Comments can also be added. When you are finished, select Save Review.

    !!! NOTE
        You configured Reviewer information during the Setup process. See Help Topic, QT - Setup under Mission Training.
...
4. Choose whether the MFR is Informational or Endorsement
...
6. Signed MFR\(s\) can be exported through the Export Folder option.
    ...
    !!! NOTE
        A Training Folder cannot be exported with unsigned MFR\(s\).
...
6. Select the sign icon to the right of the MFR sign method. An MFR can be signed with a CAC or PEX SID numbers.
...
When the student has completed the training program, you can closeout his Training Folder. A person cannot be transferred to another PEX database until the student’s course is set to completed. The Mark Completed button sets the course as complete. The Closeout Edit button populates the closeout summary form, if one is used.
...
5. If a summary form is used, scroll to the bottom of the gradesheet list and click the Closeout > Edit button.
...
6. When the Closeout dialog opens, complete the areas as required.
    ...
    1. Select the Date, Training Type, System, and Overall rating.

    2. Import a Training Script/Test. The test must already be associated to the course.

    3. Select Proficiency Ratings.

    4. Select BOIs, or add a task template as required.

    5. Select an Instructor from those assigned to the course, and a Commander.

    6. Select the Recommended Corrective Actions, if any.

    7. Enter Comments directly, or select/create a template.

    8. Enter Deviations.

    9. Reset AF 4419 clears all entries.

    10. After clicking Save, click View/Sign to sign the actual AF 4419.
...
!!! note
    The instructor must have been previously added to the course.

1. Training > Mission > Setup > Manage Instructors for Students.

2. Select your squadron and the Course Name.

3. Click Assign Instructors to Students.

4. When the dialog opens, check the desired Enrolled Students.

5. Select the Primary Instructor, Secondary Instructor, and Continuity Instructors.

6. Click OK.

7. To view and edit the assigned instructors, click the View/Edit link to the right of each Enrolled Student’s name.

#### Assign a planned instructor to a BOI

1. Training > Mission > Overview > Student Details.

2. Select the Student and the Course Name.

3. Open the desired BOI.

4. Select the Planned Instructor.
...
4. Select the Actual Instructor.
...
![binders](PEX/img/../../img/Training/Instructor Assignment Flow.jpg){: style="height:100%;width:100%"}
...
Once open, view the following controls and fields:
...
* Settings \(Check the setting items desired.\)
    ...
    * Number of Grades Displayed: 1 through 5
...
* Grade Tasks button
    ...
    * Gradeset; if the Setup Most Common Grades option is used, the Gradeset column displays only those grades set as common.
...
3. Grade the BOI using the fol owing options.
    ...
    1. Date. The Date field displays the planned date. If the date differs from the planned date determined when the student was enrolled \(with or without using a template\) PEX wil change this BOI to the selected day and leave all other BOI’s as is. If you need to edit the date:
        ...
        5. Change only this BOI to the selected date and all other BOI’s following.

        6. Change this BOI to the selected day and leave all other BOI’s as is. \(This is the gradesheet default.\)

        7. Reflow all BOI’s from the planned date of XXXX, including this BOI, forward by this many training days: Input the amount of days in a \+/- format.

        8. Apply a new template from this BOI on.

    2. Status

        1. Completed

        2. Pro-Adv: Proficiency Advancement

        3. INC\_WX: Incomplete due to WX
...
This option allows the instructor to add tasks not normally covered in the open BOI.

1. Open the Gradesheet.

2. Click More Options \(...\).

3. Select Add Additional Tasks.

4. When the task list opens, select the desired Tasks.
...
3. Select Apply Grade to Multiple Tasks.

4. When the task list opens, click Choose Grade.

5. Select the Grade, Proficiency Level and/or Knowledge Level.

6. Click Done.

7. Select the desired Tasks.

8. Click Done and return to the gradesheet.

9. Click Save and view notification "Changes have successfully saved."
...
3. Select Apply Grades to Remaining Students.

4. Select the desired students.

5. Click Save and view notification "Changes have successfully saved."
...
3. Select View Unaccomplished Tasks Log.

4. When the log opens, view Date of Notice, BOI, Task\(s\), Instructor, Reviewer Comments, Instructor \(Completion\).

5. Unconfirm, Edit or Delete as required.
...
These procedures can be accessed on a properly configured mobile device, or by selecting Training > Mission > Gradesheet from the PEX main menu on a desktop computer.
...
![binders](PEX/img/../../img/Training/Mobile Workflow.jpg){: style="height:100%;width:100%"}
...
!!! NOTE
    Currently, it is assumed your mobile device is an iPad or similar.
...
7. Download individual or multiple BOI\(s\).
    ...
    1. Click the down arrow on individual BOIs.

    2. Click the More Options \(...\) button, check multiple BOIs, and click Download.

8. In both of the above options, select the instructor.

9. The selected BOI\(s\) have downloaded to your device.

Once downloaded, you can leave the Gradesheet open for grading. We suggest saving it to your Home Screen using the Share icon and reopening when ready to grade.
...
3. Upload individual or multiple BOI\(s\).
    ...
    1. Click the up arrow on individual BOIs.

    2. Click the More Options \(...\) button, check multiple BOIs, and click Upload.
...
3. View the list of BOIs downloaded to your device.
    ...
    1. Clicking the More Options \(...\) button displays Expand All and Collapse All options.
...
* More Options \(...\) button
    ...
    * Settings \(Check the setting items desired.\)
        ...
        * Gradesheet Table Settings
            ...
            * Show Selected Item Highlight \(The selected Task row is highlighted in light blue.\)

                * Show Crosshair Highlight \(The selected column is highlighted in light blue.\)
...
5. Clicking Yes puts the entry title, or subject, on the AF6231.

6. Clicking No leaves the entry title, or subject, off the AF6231.
...
3. Assign a template Name click Add to add steps. Typically, these would be the offices to review the training folder, e.g., DOT, DOV, DO, CC, et3.
